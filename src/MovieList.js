  ...
  render() {
    return (
      <div>
        ...
        />
        {this.props.movie &&
          this.props.movie.map((movies, index) => {
            let classImg = index % 2 === 0 ? 'even' : 'odd';
            return (
              <div key={index}>
                <div className="MovieList">
                  <div className={classImg}></div>
                  <div className="movieName">{movies.movie_name}</div>
                  <div className="language">{movies.language}</div>
                  <div className="type">{movies.type}</div>
                  <div className="rating">{movies.rating}</div>
                  <div className="book">
                    <button onClick={() => this.props.book(movies)}>
                      Book
                    </button>
                  </div>
                </div>
